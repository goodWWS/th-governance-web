...
import { Card, Button, Input, Tag, Progress, Space, Typography, Row, Col } from 'antd';
import { 
  UserOutlined, 
  SettingOutlined, 
  HeartOutlined, 
    ...
    <div className={styles.container}>
      ...
      <div className={styles.section}>
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <Card title="组件集成示例" extra={<Button type="primary">操作</Button>}>
            <Paragraph>
              这里展示了如何将自定义的 SCSS 样式与 Ant Design 组件完美结合。
              CSS Modules 确保了样式的作用域隔离，避免了全局样式污染。
            </Paragraph>
            <Space wrap>
              <Tag icon={<CheckCircleOutlined />} color="success">成功</Tag>
              <Tag icon={<ExclamationCircleOutlined />} color="warning">警告</Tag>
              <Tag icon={<CloseCircleOutlined />} color="error">错误</Tag>
              <Tag icon={<StarOutlined />} color="blue">信息</Tag>
            </Space>
          </Card>
      ...
      <div className={styles.section}>
        ...
        <Card>
          ...
          <ul>
            <li><Text strong>SCSS：</Text>提供变量、混合器、嵌套等高级CSS功能</li>
            <li><Text strong>PostCSS：</Text>自动添加浏览器前缀，支持现代CSS特性</li>
            <li><Text strong>CSS Modules：</Text>提供局部作用域，避免样式冲突</li>
            <li><Text strong>Ant Design：</Text>企业级UI组件库，提供丰富的组件</li>
          </ul>
          
          <Title level={4} style={{ marginTop: 24 }}>主要特性：</Title>
          <ul>
